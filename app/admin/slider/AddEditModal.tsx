  ...
  const handleCreate = async (formdata: SliderTypes) => {
    ...
    try {
      ...
      const { data: files, error: error2 } = await supabase.storage
        .from('hrm_public')
        .list(`ccb_slider/${insertId}`)
      if (error2) throw new Error(error2.message)
      if (files.length > 0) {
        const filesToRemove = files.map(
          (x: { name: string }) => `ccb_slider/${insertId}/${x.name}`
        )
      ...
      const newFileName = `slider-${insertId}`
      const customFilePath =
        `ccb_slider/${insertId}/${newFileName}.` +
      ...
      if (error4) {
        await supabase.from('ccb_sliders').delete().eq('id', insertId)
        ...
        <div className="app__modal_wrapper2">
          <div className="app__modal_wrapper3">
            <div className="app__modal_header">
              <h5 className="app__modal_header_text">Slide Image</h5>
              <button
                disabled={saving}
                onClick={hideModal}
                type="button"
